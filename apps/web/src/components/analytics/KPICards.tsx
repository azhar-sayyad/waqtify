...
export function KPICards({ data }: KPICardsProps) {
  const { totalCompleted, overallRate, maxCurrentStreak, maxLongestStreak, dateRange } = data;
    ...
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="group relative">
        ...
        <StatCard
          ...
          accentValue={overallRate >= 70}
          icon={<BarChart2 className="w-4 h-4 text-primary" />}
          decorativeIcon={<BarChart2 className="w-32 h-32" />}
          trend={
            overallRate >= 70 ? { direction: 'up', label: 'Great consistency' }
            : overallRate >= 40 ? { direction: 'neutral', label: 'Keep building' }
      ...
      <div className="group relative">
        ...
        <StatCard
          ...
          trend={
            maxCurrentStreak >= 7 ? { direction: 'up', label: 'Week+ streak!' }
            : maxCurrentStreak > 0 ? { direction: 'neutral', label: 'Keep it up' }
            : undefined
          }
          className="relative"
        />
