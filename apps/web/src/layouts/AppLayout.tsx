  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-card/80 backdrop-blur-xl border-b border-border/50 z-50">
        <div className="h-full flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <button className="relative p-2 rounded-lg hover:bg-secondary transition-colors hidden md:block">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-border/30 hidden md:flex">
              ...
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/80 to-primary flex items-center justify-center text-sm font-bold text-primary-foreground border-2 border-primary/20 shadow-sm">
          ...
          <nav className="space-y-2">
            ...
            <div className="pt-4 border-t border-border/30 mt-4">
              <div className="flex items-center gap-3 px-4 py-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary/80 to-primary flex items-center justify-center text-base font-bold text-primary-foreground">
                  {user?.name?.[0]?.toUpperCase() || 'U'}
                </div>
      ...
      <main className="flex-1 pt-16 flex">
        {/* Sidebar (Desktop) */}
        <aside className="fixed left-0 top-16 bottom-0 w-64 bg-card/30 backdrop-blur-xl border-r border-border/30 hidden md:flex flex-col z-30">
          <div className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
            <p className="px-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3">Main Menu</p>
            {navLinks.map((link) => {
              const isActive = location.pathname === link.path;
              const Icon = link.icon;
              return (
          ...
          <div className="p-4 border-t border-border/30">
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-xl p-4">
              ...
              <p className="text-xs text-muted-foreground mb-3">Unlock advanced analytics & unlimited habits</p>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-card/80 backdrop-blur-md border-t border-border/50 z-40 flex items-center justify-around px-2 safe-area-bottom">
        {navLinks.map((link) => {
          const isActive = location.pathname === link.path;
          const Icon = link.icon;
          return (
